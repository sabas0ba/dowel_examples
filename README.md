...
本体は `0.1.0` から公開バイナリで配られている。`dowelup install 0.1.0` で
入る（Rust の toolchain は要らない）。ただし**検証に使う版は `dowel-ref` の
1行が決める**ので、追随のたびにそこを書き換える。

必要なもの: C コンパイラ（`cc`）、`ninja`、`jq`、`python3`、`git`、`cargo`、
bash 4 以降。

公開された release そのものを確かめるものは**別に置いてある**。上流の
release asset に触れる検査は「本リポジトリを変えていなければ結果も動かない」という前提と
両立しないためである。

```sh
./scripts/check-release.sh 0.1.0   # 新しい利用者が最初にすることを、そのとおりに
```
...
[F-011](docs/10-findings.md#f-011) の残っている側と、
[F-067](docs/10-findings.md#f-067)（`missing-assembler` がソースではなく目標の
見出しを指す）である。
...
| [25-asm](projects/25-asm/) | アセンブリを第3の言語として。旗の仕分け、実行可能スタック、自前の assembler、綴りの集合が閉じていること |
...
**ここで見つかるものは、最小の構成では現れない。** 報告した 67 件のうち
