...
# ここから下の計数は **app を根として組んだ場合**のものである。
# 依存（core）の検査は含まれない——使う側の build が依存の `test` を
# 組まなくなった（F-055 の修正）。それ以前は 9 件だった。
ok "app: check passes" check
runs_actions 5 "a first build runs every action (3 compiles, 1 archive, 1 link)"
...
runs_actions 3 "editing one source rebuilds it and everything downstream"
...
runs_actions 3 "editing a private header rebuilds through the depfile"
...
runs_actions 5 "editing a public header rebuilds every dependent unit"
...
# 依存の検査は、使う側を組んでも組まれない。ヘッダを触った直後でさえ
# 走らないので、これは「順序の都合でまだ来ていない」ではなく規則である。
not_rebuilt "$SCALE_TEST" "while the dependency's own test is not built by its consumer at all"
...
runs_actions 3 "touching a source without changing it still rebuilds (mtime judgement)"
...
runs_actions 3 "adding a source compiles only the new one"
rebuilt     "src_extra.c.o" "the new unit is compiled"
not_rebuilt "$CORE_SCALE"   "the existing units are not"

rm ../core/src/extra.c
runs_actions 2 "removing a source rebuilds the archive without compiling anything"
not_rebuilt "CC " "no compile action runs when only a source disappeared"

# --------------------------------------------------- 構成の切り替え

runs_actions 5 "the first release build runs every action" --config=release
...
# 公開ヘッダが検査の翻訳単位まで波及すること。**依存の側から見ると
# 組まれない**ものなので、それが組まれる側——ライブラリ自身のパッケージ
# ——で見る。組む根が変われば計数も変わるが、波及の規則は同じである。
"$DOWEL" build --no-compdb >/dev/null 2>&1
edit include/core.h
build_direct --no-compdb
rebuilt "$SCALE_TEST" "editing the public header reaches the library's own test"
rebuilt "$CORE_SCALE" "and its own units, from the package that declares them"
