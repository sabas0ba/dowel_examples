...
有無で実体が分かれるが、突き合わせる数は同じである。triple ごとに期待値を
...
ホスト付きの3つに機械の旗が要らないのは、triple を渡されたコンパイラが自分で
正しく選ぶからである。それが `[toolchain.<triple>]` に別のコンパイラを書く
意味である。

分岐は `target.os` で書く。書きたいのは「OS が無い側なら freestanding」で
あって「この triple なら」ではない。以前は対象の OS を指す語が無く triple を
数え上げていた（[F-053](../../docs/10-findings.md#f-053)、`9858932` で修正）。
数え上げは腕が対象の数だけ増えるうえ、`_` が既定なので**書き忘れた triple が
静かにホスト側の腕へ落ちる**。`target.os` は有限領域なので、腕の網羅性は
検査される。

## toolchain の置き場所（[F-054](../../docs/10-findings.md#f-054)、[#125](https://github.com/sabas0ba/dowel/issues/125)）

どのコンパイラで組むかは**ライブラリの知識**である。しかし依存の宣言は
使う側の build に効かない。これは設計として残っている——toolchain は build
全体の性質であって依存の性質ではない（ADR-0031）。

変わったのは2つある。**診断がその理由を説明するようになった**ことと、
**写しの置き場所が1つになった**ことである。

```toml
# core/dowel.toml, cli/dowel.toml, fw/dowel.toml のいずれも
[package]
toolchains = "../toolchains.toml"
```

以前は使う側それぞれが同じ表を書き写していた（支える triple の数 × 使う側の
数）。いまは1つの表を3つのパッケージが見る（[ADR-0033](https://github.com/sabas0ba/dowel/blob/main/docs/adr/0033-shared-toolchain-file.md)）。

局所の宣言は勝つ。しかも**道具1つずつ**である——`c` だけを替えれば `ar` と
`objcopy` は表から来たままになる。1つ替えるために表を書き直すなら、共有した
意味が薄れる。
...
## かつての関門: 目標を triple で絞れない（[F-055](../../docs/10-findings.md#f-055)、[#126](https://github.com/sabas0ba/dowel/issues/126)、`9858932` で修正）

`core` の検査はホスト向けである（libc を使う）。かつては `fw` をベアメタル
向けに組むとそれが混ざって落ちた。`fw` のマニフェストに誤りは無いのに、
である。ホスト付きの triple では余計に組まれるだけで無害なので、**組めない
triple が混じって初めて**現れた。
...
圏外の triple では計画に**現れない**。それでも名指しは `unsupported-target` で
断られる——名指しは要求であり、黙って何も作らない build は成功に読める。

## 束ねられない旗

機械の旗は2か所に要る——`core` の `match` の腕と、`fw` の目標である。同じ値
でなければ呼び出し規約の違う archive ができるが、揃っていることを確かめる手立てが
文法に無い（変数も文字列の連結も無い）。

`template` 種別（ADR-0035）が入ったが、**これは解かない**。雛形はパッケージ
の中のものであり、ここの2か所は別のパッケージにある。`apps/blink` のように
1つのパッケージで3つの目標が同じ旗を持つ形は、そちらで解ける。

`expect.sh` は引き続き、**2か所が実際に同じ旗を出していること**をグラフから
確かめている。
...
| | |
|---|---|
| triple ごとの toolchain | 4つの `[toolchain.<triple>]`。手元の `cc` が別の対象の物を作らない |
| triple ごとの runner | qemu-user 2つと qemu-system 1つ |
| triple ごとの旗 | `match target.os`。ソースは1つ、`#ifdef` は無し |
| triple ごとの ABI ラベル | ベアメタルだけ別 |
| 対象の分離 | ビルドディレクトリが分かれ、同名の archive の中身が違う |
| 依存の toolchain | 効かない。設計であり、診断がそう言う（F-054） |
| toolchain の共有 | `[package] toolchains` で1つの表を3つのパッケージが見る |
| 目標の triple での絞り込み | `targets` で絞る。圏外では計画に現れない |
| 派生 | ベアメタルの側は `objcopy` で生の像も出す |
| 増分 | 触った対象だけが組み直され、他の対象を道連れにしない |


## 配る（[ADR-0041](https://github.com/sabas0ba/dowel/blob/main/docs/adr/0041-install.md) / [ADR-0045](https://github.com/sabas0ba/dowel/blob/main/docs/adr/0045-offline.md)）

4つの triple へ組めることと、組んだものを**渡せる**ことは別である。使う側の
機械には dowel もビルド木も無い。

```sh
dowel install --target=aarch64-unknown-linux-gnu --prefix=/opt/dsp
```

ここでしか見られないのは**別の triple へ install する**形である。手元の機械で
組み、手元の機械に置き、動かすのは向こうの機械——なので、置いたものを qemu で
走らせて確かめる。`prefix` の中だけで動くこと、そして**配った先でも同じ答が
出ること**まで見る。各 triple のビルド木の中で同じ答が出ることは既に見てきたが、
...
pkg-config が答える。**どちらも取ってくるものではないので、この木は最初から
網に触れない。** それを性質として言えるようにするのが `--offline` である。

pkg-config は網の話ではない。手元の処理を起こして手元のファイルを読む——
ここを止めると `--offline` の意味が「することを減らす」に変わってしまう。
