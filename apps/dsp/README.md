...
## 道具立ての置き場所（[F-054](../../docs/10-findings.md#f-054)、[#125](https://github.com/sabas0ba/dowel/issues/125)）

どのコンパイラで組むかは**ライブラリの知識**である。しかし依存の宣言は
使う側の build に効かない。これは設計として残っている——道具立ては build
...
以前は使う側それぞれが同じ表を書き写していた（支える三つ組の数 × 使う側の
...
| 道具立ての共有 | `[package] toolchains` で1つの表を3つのパッケージが見る |
...
4つの三つ組へ組めることと、組んだものを**渡せる**ことは別である。使う側の
機械には dowel もビルド木も無い。
